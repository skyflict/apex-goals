...
import { saveUserGoal } from "@/services/goalsRepository";
    ...
    <div
      ...
      style={{
        ...
        background: milestone.done
          ? colors.accent
          : milestone.current
            ? "rgba(232,153,48,0.14)"
            : "rgba(255,255,255,0.05)",
        border: milestone.done
          ? "none"
          : milestone.current
            ? `1.5px solid rgba(232,153,48,0.45)`
            : `1.5px solid rgba(255,255,255,0.12)`,
        boxShadow: milestone.current
          ? `0 0 0 4px rgba(232,153,48,0.08)`
          : "none",
      }}
    >
...
const completeGoalMilestone = (goal: Goal, milestoneId: string): Goal => {
  const milestones = goal.milestones.map((milestone) => (
    milestone.id === milestoneId
      ? { ...milestone, done: true, current: false }
  ...
  const doneCount = updatedMilestones.filter((milestone) => milestone.done).length;
  ...
  return {
    ...goal,
    progress,
    nextAction: updatedMilestones[nextIndex]?.text ?? "Цель завершена",
...
export const GoalDetailScreen: React.FC = () => {
  ...
  const handleCompleteMilestone = async (milestoneId: string) => {
    const updatedGoal = completeGoalMilestone(goal, milestoneId);
    dispatch(completeMilestone({ goalId: goal.id, milestoneId }));
    ...
    setIsSaving(true);
    try {
      await saveUserGoal(user.uid, updatedGoal);
    } catch (err) {
      console.error("Failed to save milestone progress", err);
      setSaveError("Не удалось сохранить прогресс. Проверь подключение и попробуй ещё раз.");
    } finally {
      setIsSaving(false);
    }
  };
  ...
  return (
    ...
    >
      ...
      >
        <Button
          variant="icon"
          onClick={() => dispatch(setDashboardTab("home"))}
      ...
      >
        ...
        <Button
          ...
          onClick={() => {
            ...
            if (current) void handleCompleteMilestone(current.id);
          }}
        >
          <CheckIcon size={14} /> {isSaving ? "Сохраняю..." : "Готово"}
        </Button>
      </div>
      ...
      {/* Milestones */}
      <EyebrowLabel>
        Вехи · {goal.milestones.filter((m) => m.done).length} из{" "}
      ...
      <div style={{ marginTop: 18, marginBottom: 28 }}>
        {goal.milestones.map((m, i) => (
          <MilestoneItem
            ...
            onComplete={() => void handleCompleteMilestone(m.id)}
          />
        ))}
      </div>
      ...
      <AiInsight
        text={
          <>
            Ты занимаешься стабильно — молодец. При текущем темпе достигнешь
            цели на{" "}
            <span
              style={{
                color: colors.accent,
